...
if ($supervisora) {
    ...
    $supervisora = isset($estagiario->superviosr->nome);
}
...
if ($regiao) {
    ...
    $regiao = isset($estagiario->supervisor->region);
}
...
<h2 style="text-align:center; line-height: 80%; margin: 0">
    ...
    <br />
    <span style="font-size: 100%">Avaliação do estagiário(a) <?= $avaliacao->estagiario->aluno->nome ?? $estagiario->aluno->nome ?></span>
    <br />
</h2>

<p><span style="font-size: 100%; text-align: justify; font-weight: normal">Campo de estágio <?= $avaliacao->estagiario->instituicao->instituicao ?? $estagiario->instituicao->instituicao ?>. Supervisor(a) <?= $supervisora ?? '____________________' ?>, Cress <?= $cress ?? '_____' ?>. Período de estágio <?= $avaliacao->estagiario->periodo ?? $estagiario->periodo ?>. Nível: <?= $avaliacao->estagiario->nivel ?? $estagiario->nivel ?>. Supervisão acadêmica: <?= $professora ?? '____________________' ?></span></p>
...
<div class="container">
            ...
            <div class="avaliacoes view content">
                <table>
                    <tr>
                        ...
                        <td><?php
                            switch ($avaliacao->avaliacao1 ?? $avaliacao1):
                                case 0:
                                    echo "Ruim";
                                    break;
                                case 1:
                    ...
                    <tr>
                        ...
                        <td><?php
                            switch ($avaliacao->avaliacao2 ?? 'avaliacao2'):
                                case 0:
                                    echo "Ruim";
                                    break;
                                case 1:
    ...
    <table class="table" style="width: 100%; background-color: white;">
        <tr>
            ...
            <td style="width: 33%"><span style="font-size: 100%; text-decoration: overline"><?= $avaliacao->estagiario->aluno->nome ??  $estagiario->aluno->nome ?></span></td>
            <td style="width: 33%"><span style="font-size: 100%; text-decoration: overline"><?= $supervisora ?? '____________________' ?></span></td>
        </tr>

        <tr>
            <td style="width: 33%"></td>
            <td style="width: 33%"><span style="font-size: 100%">DRE: <?= $avaliacao->estagiario->aluno->registro ?? $estagiario->aluno->registro ?></span></td>
            <td style="width: 33%"><span style="font-size: 100%">CRESS <?= $regiao ?? '_____' ?>ª Região <?= $cress ?? '_____' ?></span></td>
        </tr>
    </table>
</div>
